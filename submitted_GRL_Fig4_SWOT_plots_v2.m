...
adcp2_u = data_Leg.u(28,ind{2});
adcp2_v = data_Leg.v(28,ind{2});
...
% Quiver settings
%% ============================================================
adcp_depth_ind = 28;                         % depth index for ADCP vectors
target_depth_q = double(data_Leg.depth(adcp_depth_ind));

qskip = 8;                                   % model-vector thinning; increase if crowded
vec_km_per_ms = 15;                          % arrow length scale: 1 m/s = 15 km
model_quiver_color = [0.05 0.05 0.05];       % dark gray/black
model_quiver_lw = 1.0;
...
% Apply the same map cropping as the submitted figure workflow
irow0 = 36;
...
u_swotda = (data_swotda.u(:,:,19,it_swotda)'+ data_swotda.u(:,:,20,it_swotda)')/2;
v_swotda = (data_swotda.v(:,:,19,it_swotda)'+ data_swotda.v(:,:,20,it_swotda)')/2;
...
u_joint = (data_joint.u(:,:,19,it_joint)'+ data_joint.u(:,:,20,it_joint)')/2;
v_joint = (data_joint.v(:,:,19,it_joint)'+ data_joint.v(:,:,20,it_joint)')/2;
...
% Pick depth closest to ADCP vector depth
adcp_depth_ind = 5;  %31;
target_depth_q = double(data_Leg.depth(adcp_depth_ind));

[~, izq_swotda] = min(abs(double(data_swotda.depth) - target_depth_q));
[~, izq_joint]  = min(abs(double(data_joint.depth)  - target_depth_q));

% Extract model velocity at selected depth and time
u_swotda = squeeze(data_swotda.u(:,:,izq_swotda,it_swotda));   % lon x lat
v_swotda = squeeze(data_swotda.v(:,:,izq_swotda,it_swotda));

u_joint = squeeze(data_joint.u(:,:,izq_joint,it_joint));
v_joint = squeeze(data_joint.v(:,:,izq_joint,it_joint));

% Interpolate model velocities onto ADCP track
u_intp_swotda = interp2(double(data_swotda.lon), double(data_swotda.lat), ...
    double(u_swotda'), double(adcp2_lon), double(adcp2_lat), 'linear', NaN);

v_intp_swotda = interp2(double(data_swotda.lon), double(data_swotda.lat), ...
    double(v_swotda'), double(adcp2_lon), double(adcp2_lat), 'linear', NaN);

u_intp_joint = interp2(double(data_joint.lon), double(data_joint.lat), ...
    double(u_joint'), double(adcp2_lon), double(adcp2_lat), 'linear', NaN);

v_intp_joint = interp2(double(data_joint.lon), double(data_joint.lat), ...
    double(v_joint'), double(adcp2_lon), double(adcp2_lat), 'linear', NaN);

%% ============================================================
% Quiver manual scaling
% Larger value = longer vectors
%% ============================================================
qskip = 2;                 % use 1 if you want every ADCP point
vec_km_per_ms = 50;        % 1 m/s appears as 50 km on the map

lat_ref_q = mean(axis_def(3:4));
u_scale = vec_km_per_ms / (111 * cosd(lat_ref_q));   % deg lon per m/s
v_scale = vec_km_per_ms / 111;                       % deg lat per m/s

u_intp_swotda_plot = u_intp_swotda * u_scale;
v_intp_swotda_plot = v_intp_swotda * v_scale;

u_intp_joint_plot = u_intp_joint * u_scale;
v_intp_joint_plot = v_intp_joint * v_scale;

adcp2_u_plot = adcp2_u * u_scale;
adcp2_v_plot = adcp2_v * v_scale;
...
% fig = figure('Color','w','Units','pixels','Position',[100 120 1300 400]);
% tl = tiledlayout(1,3,'TileSpacing','compact','Padding','compact');
...
% Model quiver: SWOT DA
q1 = quiver(ax1, ...
    adcp2_lon(1:qskip:end), ...
    adcp2_lat(1:qskip:end), ...
    u_intp_swotda_plot(1:qskip:end), ...
    v_intp_swotda_plot(1:qskip:end), ...
    0, ...
    'Color','r', ...
    'LineWidth',1.5);
...
% CTD stations
plot(ax1, CTD_lon, CTD_lat, 'o', ...
    'MarkerFaceColor', [0.5210 0.0860 0.8190], ...
    'MarkerEdgeColor', [0.5210 0.0860 0.8190], ...
    'MarkerSize', 6)
...
% ===== Reference vector =====
rectangle(ax1, 'Position', [0.88 39.28 0.42 0.12], ...
...
qref = quiver(ax1, lon_ref, lat_ref, ...
    uref * u_scale, vref * v_scale, ...
    0, ...
    'Color','r', ...
    'LineWidth',1.8);
qref.ShowArrowHead = 'off';

text(ax1, lon_ref + uref*u_scale + 0.03, lat_ref, '0.2 m s^{-1}', ...
...
% Model quiver: Joint DA
q2 = quiver(ax2, ...
    adcp2_lon(1:qskip:end), ...
    adcp2_lat(1:qskip:end), ...
    u_intp_joint_plot(1:qskip:end), ...
    v_intp_joint_plot(1:qskip:end), ...
    0, ...
    'Color','r', ...
    'LineWidth',1.5);
...
title(ax2, ['Joint DA: ' datestr(t_joint,'dd-mmm HH:MM')], ...
    'FontWeight','bold','FontSize',13)
...
q3 = quiver(ax3, ...
    adcp2_lon(1:qskip:end), ...
    adcp2_lat(1:qskip:end), ...
    adcp2_u_plot(1:qskip:end)', ...
    adcp2_v_plot(1:qskip:end)', ...
    0, ...
    'Color','r', ...
    'LineWidth',1.5);
...
title(ax3, ['SWOT: ' datestr(t_swot,'dd-mmm HH:MM')], ...
    'FontWeight','bold','FontSize',13)
...
deg_lon = km / (111 * cosd(lat_ref_bar));
lon0 = 2.65;
lat0 = 39.65;
...
legend([p1,p3,p2], {'ADCP','Glider','CTD'}, ...
