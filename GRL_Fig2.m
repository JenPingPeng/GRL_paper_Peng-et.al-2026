...
% === Your Updated Script with Movie Selection Option ===
...
%% the following code has several parts: 

%% 2. temporal evolution of joint DA performance
%% 3: the entire model performance over DA phase
%% 5: spatial varability of SSH across each simulation
%% they can not run at once, PC would crush.
%% so, select number here to run each of them
plot_index = 5; % 3 or 2 
...
version_labels = { % new simuluation    
    'GEN', ...
    'SWOT DA', ...    
    'In-situ DA', ...
    'Joint DA'... 
...
%% ADD ADCP and glider
[info, glider1] = read_nc_file_struct(['/media/jpeng/Extreme SSD/01_IMEDEA_2025/Simulations/Cruise_data/Glider/dep0007_sdeep09_scb-sldeep009_L2_2023-04-25_data_dt.nc']);
...
[finf_Leg data_Leg] = read_nc_file_struct([adcp_path 'ADCP_OS150_FaSt-SWOT_LEG1.nc']);
data_Leg.Time = data_Leg.time + datenum(2023,1,1);
% to find out which transect is better to show as validation
    ...
    %% for ii = 1 : 16 % For getting RMSE for table 1
        ...
        for k = 1:length(dir_SWOT) % for model
            if contains(dir_SWOT(k).name, ind_name_swot)
                [info, data_swot] = read_nc_file_struct([SWOT_path_new dir_SWOT(k).name]);
        ...
        for j = j_val %1: 2 : Nt
            ...
            clf(fig4);
            set(fig4,'Color','w','Position',[80 120 1700 420]);
            tl = tiledlayout(fig4,1,5,'TileSpacing','compact','Padding','compact');
...
text(lon0+deg_lon/2,lat0,'Mallorca','HorizontalAlignment','center','FontSize',16)
        ...
        print(['/media/jpeng/Extreme SSD/01_IMEDEA_2025/Figures/GRL/Fig2_upper'], '-dpng', '-r300');
...
%% revise this
ti = [8 8 7 7 6];
if plot_index == 2 %% this is correct one,
...
tl = tiledlayout(2,5,'TileSpacing','compact','Padding','compact');
    ...
    zeta_bot = data_joint.zeta(:,:,ti(i))'; %% this is the problem
...
if plot_index == 3 %JP
    ...
    % Plot
    %% --------------------------------------------------------
    figure('color','w','position',[200 200 1250 350]); hold on

    p1 = plot(tplot, RMSE_bot(:,1), 'LineWidth',2.2, ...
        'Color',[0.2310 0.6660 0.1960]);

    p2 = plot(tplot, RMSE_bot(:,2), 'LineWidth',2.2, ...
        'Color',[0.5210 0.0860 0.8190]);

    p3 = plot(tplot, RMSE_bot(:,3), 'LineWidth',2.2, ...
...
%1:3:18
    ...
    legend([p1,p2,p3], {'SWOT DA','In-situ DA','Joint DA'}, ...
           'Location','northeast', ...
           'Box','off')

    text(tplot(1), 4.3, '(p)', 'FontWeight','bold', 'FontSize',14)
    ...
    print(['/media/jpeng/Extreme SSD/01_IMEDEA_2025/Figures/GRL/Fig2_lower_new'], '-dpng', '-r300');      
