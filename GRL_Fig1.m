...
%% CTD
    ...
    for i = 3 : 14
        ...
        lat1(i,1) = str2double(tok{1}) + str2double(tok{2})/60; if tok{3}=='S', lat = -lat; end
        
        tok = regexp(lon1_str,'(\d+)\s+(\d+\.?\d*)\s*([EW])','tokens','once');
        lon1(i,1) = str2double(tok{1}) + str2double(tok{2})/60; if tok{3}=='W', lon = -lon; end     
    end
    ...
    CTD_dir = dir(CTD2_path);
        ...
        lat2(i,1) = str2double(tok{1}) + str2double(tok{2})/60; if tok{3}=='S', lat = -lat; end
        
        tok = regexp(lon2_str,'(\d+)\s+(\d+\.?\d*)\s*([EW])','tokens','once');
        lon2(i,1) = str2double(tok{1}) + str2double(tok{2})/60; if tok{3}=='W', lon = -lon; end     
    end
    % figure; hold on
    % plot(lon1,lat1,'bo')
    % plot(lon2,lat2,'ko')
...
%% ADCP
    ...
    [finf_Leg data_Leg] = read_nc_file_struct([adcp_path 'ADCP_OS150_FaSt-SWOT_LEG2.nc']);
...
ax3 = axes('Position',[0.08 0.08 0.36 0.36]);
    ...
    [info, glider1] = read_nc_file_struct(['/media/jpeng/Extreme SSD/01_IMEDEA_2025/Simulations/Cruise_data/Glider/dep0040_sdeep01_scb-sldeep001_L2_2023-04-25_data_dt.nc']);
...
% nexttile(1)
    ...
    [info, data_swot] = read_nc_file_struct([SWOT_path_new dir_SWOT(34).name]); % JP
    ...
    p2 = plot(lon1,lat1,'o','MarkerFaceColor',[0.5210    0.0860    0.8190]);
...
hour = 6;
    ...
    %% see your defined line 28.8:0.05:29.1 and count the lines munually
